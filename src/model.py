...
class SWMSA(WMSA):
    ...
    def get_mask(self, shape: torch.Size) -> torch.Tensor:
        ...
        *_, H, W = shape
        ...
        mask_windows = rearrange(self.partition(img_mask), "MM nW -> nW MM")
...
class RelativePositionBias(nn.Module):
    def __init__(self, M: int, nH: int):
        ...
        relative_coords = coords.unsqueeze(1) - coords.unsqueeze(0)  # MM MM 2
        relative_coords[..., 0:2] += M - 1  # Shift coords to start from 0
        relative_coords[..., 0] *= 2 * M - 1  # Unique row, col pairs per row
        self.index = relative_coords.sum(-1).flatten()  # M^4

    def __radd__(self, x: torch.Tensor) -> torch.Tensor:
        bias = self.bias_table[self.index].unflatten(0, (self.M**2, self.M**2))
        return x + rearrange(bias, "... nH -> nH ...").to(x.device)
