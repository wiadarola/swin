...
class RelativePositionBias(nn.Module):
    def __init__(self, M: int, nH: int):
        ...
        coords = torch.cartesian_prod(torch.arange(M), torch.arange(M))  # (M^2, 2)
        relative_coords = coords[:, None, :] - coords[None, :, :]  # (M^2, M^2, 2)
        relative_coords += M - 1  # shift to >= 0
        relative_coords[..., 0] *= 2 * M - 1

        self.index: torch.Tensor
        self.register_buffer("index", relative_coords.sum(-1).reshape(-1))  # M^4

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        flat = torch.index_select(self.bias_table, 0, self.index)  # MMMM nH
        bias = rearrange(flat, "(MM1 MM2) nH -> nH MM1 MM2", MM1=self.MM)
        return x + bias
